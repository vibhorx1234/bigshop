...
const Refrigerator = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const data = await getProductsByCategory('Refrigerator');
      setProducts(data);
    } catch (error) {
  ...
  if (loading) {
    return <Loader message="Loading Refrigerators..." />;
  }
        ...
        <Container maxWidth="xl">
          ...
          >
            ...
            <Typography variant="body1" color="text.secondary" paragraph>
              Side by side, French door, and double door refrigerators
            </Typography>
          </motion.div>
          ...
          <Paper elevation={0} sx={{ p: 2, mb: 3, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={12} md={8}>
                <SearchBar onSearch={handleSearch} placeholder="Search refrigerators..." />
              </Grid>
